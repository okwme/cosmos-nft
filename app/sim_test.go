...
func testAndRunTxs(app *CosmicApp, config simulation.Config) []simulation.WeightedOperation {
...
// Profile with:
// /usr/local/go/bin/go test -benchmem -run=^$ github.com/cosmos/cosmos-sdk/CosmicApp -bench ^BenchmarkFullAppSimulation$ -Commit=true -cpuprofile cpu.out
func BenchmarkFullAppSimulation(b *testing.B) {
	...
	gapp := NewCosmicApp(logger, db, nil, true, simapp.FlagPeriodValue, interBlockCacheOpt())
...
func TestFullAppSimulation(t *testing.T) {
	...
	gapp := NewCosmicApp(logger, db, nil, true, simapp.FlagPeriodValue, fauxMerkleModeOpt)
	require.Equal(t, "CosmicApp", gapp.Name())
...
func TestAppImportExport(t *testing.T) {
	...
	app := NewCosmicApp(logger, db, nil, true, simapp.FlagPeriodValue, fauxMerkleModeOpt)
	...
	newApp := NewCosmicApp(log.NewNopLogger(), newDB, nil, true, simapp.FlagPeriodValue, fauxMerkleModeOpt)
...
func TestAppSimulationAfterImport(t *testing.T) {
	...
	newApp := NewCosmicApp(log.NewNopLogger(), newDB, nil, true, 0, fauxMerkleModeOpt)
	require.Equal(t, "CosmicApp", newApp.Name())
...
func TestAppStateDeterminism(t *testing.T) {
	...
	for i := 0; i < numSeeds; i++ {
		...
		for j := 0; j < numTimesToRunPerSeed; j++ {
			logger := log.NewNopLogger()
			db := dbm.NewMemDB()
			app := NewCosmicApp(logger, db, nil, true, simapp.FlagPeriodValue, interBlockCacheOpt())
